...
static int tcp_seq_acceptable(tcp_t *tcp, tcp_seg_t *seg) {
    ...
    if (seg->seq_len == 0) {
        if (rcv_win == 0) {
            ...
            int v = TCP_SEQ_LE(tcp->rcv.nxt, seg->seq) 
                    && TCP_SEQ_LT(seg->seq, tcp->rcv.nxt + rcv_win);
            return v;
        }
    } else {
        if (rcv_win == 0) {
            ...
            v |= TCP_SEQ_LE(tcp->rcv.nxt, slast) 
                    && TCP_SEQ_LT(slast, tcp->rcv.nxt + rcv_win);
            return v;
        }
    }
}
