...
net_err_t icmpv4_out_unreach(ipaddr_t *dest_ip, ipaddr_t *src, uint8_t code, pktbuf_t *ip_buf) {
    ...
    pktbuf_t *new_buf = pktbuf_alloc(copy_size + sizeof(icmpv4_hdr_t) + 4);//这里的4是有4字节未用空间
    if (!new_buf) {
    ...
    pkt->hdr.code = code;
    pkt->hdr.checksum = 0;
    pkt->reverse = 0;//未使用的4字节

    pktbuf_reset_acc(ip_buf);

    //从发过来的包中原样拷贝数据
    pktbuf_seek(new_buf, sizeof(icmpv4_hdr_t) + 4);//定位到写入位置
    ...
    if (err < 0) {
        dbg_error(DBG_ICMPv4, "send icmp unreach failed.");
        pktbuf_free(new_buf);
        return err;
    }
